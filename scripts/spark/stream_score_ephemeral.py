...
def run(run_seconds: int = None):
    """
    Run the streaming queries. If run_seconds is None -> run continuously.
    If run_seconds is an integer -> run for up to run_seconds seconds and stop.
    """
    spark = create_spark()
    ...
    try:
        if run_seconds is None:
            # original blocking behavior - run forever
            parquet_query.awaitTermination()
            kafka_query.awaitTermination()
            console_q.awaitTermination()
        else:
            # wait up to run_seconds for micro-batches, then stop
            # awaitTermination accepts seconds (int)
            parquet_query.awaitTermination(run_seconds)
            kafka_query.awaitTermination(run_seconds)
            console_q.awaitTermination(run_seconds)
    except Exception as e:
        # log to stderr for NiFi / wrapper visibility
        import sys, traceback
        traceback.print_exc(file=sys.stderr)
    finally:
        # stop queries and spark cleanly
        try:
            parquet_query.stop()
        except Exception:
            pass
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Stream scorer with optional ephemeral run")
    parser.add_argument("--run-seconds", type=int, default=None,
                        help="If set, run streaming for this many seconds then exit.")
    args = parser.parse_args()
    run(args.run_seconds)
